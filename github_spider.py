...
def handle_rate_limit(response):
    if response.status_code == 403:
        ...
        sleep_time = (
            max(0, reset_time - int(time.time())) + 5
...
def star_repository(username, repo_name):
    ...
    if response.status_code == 204:
        ...
        print(
            f"Failed to star repository: {username}/{repo_name}. Status code: {response.status_code}"
        )


# Function to display profile data
def display_profile_data(user_profile_data):
    if user_profile_data:
        print(f"Name: {user_profile_data.get('name')}")
        print(f"Company: {user_profile_data.get('company')}")
        print(f"Location: {user_profile_data.get('location')}")
        print(f"Public Repos: {user_profile_data.get('public_repos')}")
        print(f"Followers: {user_profile_data.get('followers')}")
        print(f"Following: {user_profile_data.get('following')}")
    else:
